...
def set_pretrained_weight(model,nb_conv, path_to_weight):
    ...
    for i in range(1, nb_conv+1):
        dense_layer = model.get_layer('dense_' + str(i)) ## convolusional layer
        ...
        if len(dense_layer.get_weights()) > 1: ## with bias
            bias   = weight_reader.read_bytes(np.prod(dense_layer.get_weights()[1].shape))
            kernel = weight_reader.read_bytes(np.prod(dense_layer.get_weights()[0].shape))
            kernel = kernel.reshape(list(reversed(dense_layer.get_weights()[0].shape)))
            kernel = kernel.transpose([2,3,1,0])
            dense.set_weights([kernel, bias])
        else: ## without bias
            kernel = weight_reader.read_bytes(np.prod(dense.get_weights()[0].shape))
            kernel = kernel.reshape(list(reversed(dense.get_weights()[0].shape)))
            kernel = kernel.transpose([2,3,1,0])
